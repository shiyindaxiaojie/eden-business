package com.puyiwm.montnets.spring.boot.sms.core;
...
import com.puyiwm.montnets.spring.boot.sms.constant.MontnetsPlatform;
...
import com.puyiwm.spring.framework.error.ClientException;
import com.puyiwm.spring.framework.error.ThirdServiceException;
import com.puyiwm.spring.framework.error.util.AssertEnhancer;
import com.puyiwm.spring.integration.sms.core.*;
...
public class MontnetsSmsTemplate implements SmsTemplate, InitializingBean {
	...
	public SendMultiSmsResponse multiSend(SendMultiSmsRequest request) {
		...
		for (SendMultiSmsRequest.MultiSms multiSms : multiSmsList) {
			ClientErrorType.notNull(multiSms.getPhoneNumber(), "400", "发送梦网短信的接收号码不能为空");
